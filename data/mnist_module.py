...
class MNISTDataModule(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        KMNIST(self.data_dir, train=True, download=True, transform=transform_lib.ToTensor())
        KMNIST(self.data_dir, train=False, download=True, transform=transform_lib.ToTensor())
    ...
    def val_dataloader(self, batch_size=32, transforms=None):
        ...
        dataset = MNIST(self.data_dir, train=False, download=True, transform=transforms)
        dataset2 = KMNIST(self.data_dir, train=False, download=True, transform=transforms)
        dataset2.targets[:] = 10
        dataset.data = torch.vstack([dataset.data,dataset2.data[:10000,...]])
        dataset.targets = torch.hstack([dataset.targets,dataset2.targets[:10000]])
        train_length = len(dataset)
